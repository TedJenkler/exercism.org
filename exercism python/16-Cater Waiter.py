...
def clean_ingredients(dish_name, dish_ingredients):
    clean_set = set(dish_ingredients)
    return (dish_name, clean_set) 
...
def tag_special_ingredients(dish):
    ...
    marked_ingredients = ingredients_set.intersection(SPECIAL_INGREDIENTS)
    return (dish[0], marked_ingredients)
...
def separate_appetizers(dishes, appetizers):
    all_dishes = set(dishes)
    appetizer_set = set(appetizers)
    main_dishes = all_dishes.difference(appetizer_set)
